files = dir(fullfile('G:\420_MEN\library\','*.jpg'));
target=imread('G:\420_MEN\raw.jpg');
...
for i = 1:lengthFiles
    try
        Img = imread(strcat('G:\420_MEN\library\',files(i).name));%文件所在路径
        %改变大小
        ...
        disp(strcat('G:\420_MEN\library\',files(i).name)) %打印文件路径
        error=error+1;
    end
end
...
for i=1:10:x
    for j=1:10:y
        ...
        %disp(mini)
        Img = imread(strcat('G:\420_MEN\library\',files(mini).name));%文件所在路径
        newimg=imresize(Img,[10,10]);
        for q=0:9
            for p=0:9
